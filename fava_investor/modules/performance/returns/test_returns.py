...
def dates_from_timeline(timeline):
    ...
    """
    return [(segment.begin.date, segment.end.date) for segment in timeline]
...
class TestReturnsFunctions(test_utils.TestCase):
    ...
    def test_regexps_to_accounts(self):
        ...
        self.assertEqual(
            {
                "Assets:US:Prosper:InFunding",
                "Assets:US:Prosper:Cash",
                "Assets:US:Prosper:FundsLent",
            },
            acc_assets,
        )
        ...
        self.assertEqual(
            {
                "Income:US:Prosper:LateFeesRecv",
                "Income:US:Prosper:ChargedOff",
                "Expenses:Financial:Fees:Prosper",
                "Income:US:Prosper:LoanInterest",
            },
            acc_intflows,
        )

        self.assertEqual(
            {"Equity:Opening-Balances", "Assets:US:TD:Checking"}, acc_extflows
        )

        self.assertEqual(None, acc_internalize)
    ...
    def test_compute_timeline_and_returns(self):
        ...
        returns.compute_timeline_and_returns(
            self.entries,
            self.options_map,
            "Equity:Internalized",
            acc_assets,
...
class TestReturnsConstrained(test_utils.TestCase):
    ...
    def setUp(self, entries, errors, _):
        ...
        self.assertFalse(errors)
        self.assets = {"Assets:US:Investments:ACME", "Assets:US:Investments:Cash"}
...
class TestReturnsExampleScript(unittest.TestCase):
    ...
    def test_returns_example_script(self):
        ...
        script_name = path.join(
            test_utils.find_repository_root(__file__), "examples", "example.returns.py"
        )
